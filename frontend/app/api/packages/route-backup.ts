...
export async function GET(request: NextRequest) {
  try {
    ...
    const cacheKey = `packages:${currency}:${activeOnly}${timestamp ? `:${timestamp}` : ''}`;
    ...
    if (includeSchedule) {
      try {
        ...
        if (scheduleResponse.success && scheduleResponse.data) {
          ...
          console.log('✅ Retrieved schedule data for packages:', scheduleData.length, 'schedules');
        }
      } catch (error) {
        console.warn('⚠️ Failed to fetch schedule data for packages:', error);
      }
    }
    ...
    const transformedPackages = packages.map(pkg => {
      ...
      console.log(`Package ${pkg.name}: currency=${currency}, found price:`, packagePrice);
