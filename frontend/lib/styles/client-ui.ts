// Centralized styling tokens and classnames for the Client portal layout
// Mirrors `teacher-ui.ts` to keep structure consistent while allowing theme differences.
...
export const clientUI = {
  ...
  sidebar: {
    container: 'flex w-64 flex-col bg-[var(--color-sidebar-800)] border-r border-[var(--color-border-500)]',
    header: 'flex h-16 items-center justify-center border-b border-[var(--color-border-500)]',
    section: 'px-4 py-4 border-b border-[var(--color-border-500)]',
    nav: 'flex-1 space-y-1 px-4 py-6',
    footer: 'border-t border-[var(--color-border-500)] p-4',
    brandTitle: 'text-xl font-bold text-[var(--color-text-inverse)]',
    brandTagline: 'text-xs text-[var(--color-text-secondary)]',
  },
  header: {
