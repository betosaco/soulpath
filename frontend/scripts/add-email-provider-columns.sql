-- Add Email Provider, WhatsApp, and Instagram columns to communication_config table
-- Run this script manually or through your database admin tool

-- Add Email provider columns
...
COMMENT ON COLUMN communication_config.email_provider IS 'Email service provider (brevo or resend)';
COMMENT ON COLUMN communication_config.resend_api_key IS 'Resend email service API key';
...
AND (
  ...
  column_name LIKE '%telegram%' OR
  column_name LIKE '%whatsapp%' OR
  column_name LIKE '%instagram%'
)
