...
async function createScheduleSlots() {
  try {
    console.log('🔍 Creating schedule slots for the current booking system...\n');

    // Check if there are any existing schedule slots
    const existingSlots = await prisma.scheduleSlot.findMany({
      take: 1
    });

    if (existingSlots.length > 0) {
      console.log('✅ Schedule slots already exist in the database');
