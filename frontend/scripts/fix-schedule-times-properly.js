...
async function fixScheduleTimesProperly() {
  try {
    console.log('🔄 Fixing schedule times to store EST properly...');
    ...
    // Create new schedule pattern with correct EST times
    // We need to store EST times as UTC, so EST 08:15 = UTC 13:15
    const newSchedules = [
...
fixScheduleTimesProperly()
  .then(() => {
    console.log('\n✅ Schedule times fix completed!');
    process.exit(0);
  })
  .catch((error) => {
