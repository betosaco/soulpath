// Test individual user with multiple packages and their own scheduled sessions
const testData = {
  customerInfo: {
  ...
  scheduleDetails: [
    ...
    },
    {
      ...
      serviceType: "Restorative Yoga",
      venue: "MatMax Yoga Studio"
    }
  ]
};

async function testIndividualMultiplePackagesSchedules() {
  try {
    console.log('🧪 Testing individual user with multiple packages and schedules...');
    console.log('📧 Will send email to:', testData.customerInfo.email);
    console.log('📦 Packages:', testData.items.length);
    ...
    if (response.ok) {
      ...
      console.log('📬 Check your email inbox for the multiple packages with schedules confirmation');
...
testIndividualMultiplePackagesSchedules();
